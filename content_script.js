...
function parseEvents(eventList) {
  ...
  for (var i = 0; i < eventList.length; i++) {
      if (eventList[i][0] != 'A' && eventList[i][0] != 'O') {
        ...
        if (unformattedDate[1] != null) {
          date = months[unformattedDate[0]] + "/" + unformattedDate[1].slice(0, unformattedDate[1].length-2) + "/" + unformattedDate[2];
        }
        unformattedTime = unformattedTime.split(" ")
        time = [unformattedTime[0] + ":00 " + unformattedTime[1], unformattedTime[3] + ":00 " + unformattedTime[4]];
        parsedEvents.push({ 
          "title": title,
